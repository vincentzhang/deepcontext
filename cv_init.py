  ...
  if 'solver' not in locals():
    ...
    while True:
      if force_quit:
        break
      # Hyperparams
      lr_val = 10 ** np.random.uniform(-6,-1)
      weight_decay_val = 10 ** np.random.uniform(-6,-2)
      decay_ratio =  np.random.uniform(0,1)

      # Print out information
      print "CV step %d, Start with parameter" % cv_step
      print "Learning rate = %f" % lr_val
      print "Regularization = %f" % weight_decay_val
      print "LR decay ratio = %f" % decay_ratio

      # msra filler
      solver_file = outdir + 'hyperparam_cv_solver.prototxt'
      ut.mkorender('solver_cv_mko.prototxt', solver_file,
                   base_lr=lr_val, outdir=deepcontext_config.out_dir,
                   weight_decay=weight_decay_val, momentum=0.9,
                   gamma=decay_ratio, fnm='network_conv_sketch_no_bn_no_freeze_msra_filler.prototxt',
                   solver_type='SGD')
